...
static BOOL kCreateApplicationPanelWindow(void) {
    ...
    // 애플리케이션 패널 왼쪽에 GUI 태스크의 리스트를 보여주는 버튼을 표시
...
static void kDrawDigitalClock(QWORD qwWindowID) {
    ...
    kDrawRect(qwWindowID, stUpdateArea.iX1, stUpdateArea.iY1, stUpdateArea.iX2,
        stUpdateArea.iY2, APPLICATIONPANEL_COLOR_INNERLINE, FALSE);
...
static BOOL kProcessApplicationPanelWindowEvent(void) {
    ...
    while(1) {
        ...
        case EVENT_MOUSE_LBUTTONDOWN:
            ...
            // 왼쪽 버튼이 애플리케이션 패널 버튼 내부에 눌리면 애플리케이션 리스트 윈도우 표시
...
static BOOL kCreateApplicationListWindow(void) {
    ...
    // 윈도우 위치는 애플리케이션 패널의 버튼 아래로 설정
    iX = gs_stApplicationPanelData.stButtonArea.iX1;
    iY = gs_stApplicationPanelData.stButtonArea.iY2 + 5;

    // 아이템 개수와 최대 길이로 애플리케이션 리스트 윈도우 생성
    // 애플리케이션 윈도우는 윈도우 제목 표시줄이 필요 없으므로 속성은 널로 전달
...
static BOOL kProcessApplicationListWindowEvent(void) {
    ...
    while(1) {
        ...
        case EVENT_MOUSE_LBUTTONDOWN:
            ...
            // 애플리케이션 패널에 마우스 왼쪽 버튼이 눌렸다는 메시시 전송하여 처리
