...
BOOL kInitializeFileSystem(void) {
    ...
    // 하드 디스크 초기화가 실패하면 8MB 크기의 램 디스크 생성
...
// MINT 파일 시스템이면 파일 시스템에 관련된 각종 정보를 읽어서 자료구조에 삽입
BOOL kMount(void) {
    MBR *pstMBR;
...
BOOL kFormat(void) {
    ...
    // 하드 디스크의 정보를 얻어서 하드 디스크의 총 섹터 수 구함
    ...
    // 최대 클러스터 수에 맞춰 클러스터 링크 테이블 섹터 수 계산
    ...
    // 예약된 영역은 현재 사용 X -> 디스크 전체 영역에서 MBR 영역과
    // 클러스터 링크 테이블 영역의 크기를 뺀 나머지가 실제 데이터 영역이 됨
    ...
    // 실제 사용 가능한 클러스터 수에 맞춰 다시 한번 계산
    dwClusterLinkSectorCount = (dwClusterCount + 127) / 128;

    // 계산된 정보를 MBR에 덮어 쓰고, 루트 디렉터리 영역까지 모두 0으로 초기화하여 파일 시스템 생성
    // MBR 영역 읽기
    if(gs_pfReadHDDSector(TRUE, TRUE, 0, 1, gs_vbTempBuffer) == FALSE) {
        kUnlock(&(gs_stFileSystemManager.stMutex));
        return FALSE;
...
// 클러스터 링크 테이블 내의 오프셋에서 한 섹터를 읽음, 내부적으로 사용하는 함수, 캐시 사용 안함
...
static DWORD kFindFreeCluster(void) {
    ...
    for(i = 0; i < gs_stFileSystemManager.dwClusterLinkAreaSize; i++) {
        ...
        // 읽어야 할 클러스터 링크 테이블의 섹터 오프셋을 구해서 읽음
...
static BOOL kSetClusterLinkData(DWORD dwClusterIndex, DWORD dwData) {
    ...
    // 한 섹터에 128개의 클러스터 링크가 들어가므로 128로 나누면 섹터 오프셋 구할 수 있음
    dwSectorOffset = dwClusterIndex / 128;

    // 해당 섹터 읽어서 링크 정보를 설정한 후 다시 저장
...
static int kFindDirectoryEntry(const char *pcFileName, DIRECTORYENTRY *pstEntry) {
    ...
    // 루트 디렉터리 루프 돌면서 파일 이름이 일치하는 엔트리 반환
    pstRootEntry = (DIRECTORYENTRY*)gs_vbTempBuffer;
    for(i = 0; i < FILESYSTEM_MAXDIRECTORYENTRYCOUNT; i++) {
...
BOOL kFlushFileSystemCache(void) {
    ...
    for(i = 0; i < iCacheCount; i++) {
        ...
        if(pstCacheBuffer[i].bChanged == TRUE) {
            ...
            // 버퍼의 내용을 모두 하드 디스크에 썼으므로 변경되지 않은 것으로 설정
