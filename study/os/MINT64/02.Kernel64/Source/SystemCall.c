...
void kInitializeSystemCall(void) {
    ...
    // IA32_LSTAR MSR 설정, SystemCallEntryPoint 함수 어드레스로 지정
    kWriteMSR(0xC0000082, 0, (QWORD)kSystemCallEntryPoint);

    // IA32_FMASK MSR 설정
    // RFLAGS 레지스터 값 변경은 없으므로 0x00으로 설정하여 값 유지
    kWriteMSR(0xC0000084, 0, 0x00);
}

// 시스템 콜 번호화 파라미터 자료구조를 이용하여 시스템 콜 처리
QWORD kProcessSystemCall(QWORD qwServiceNumber, PARAMETERTABLE *pstParameter) {
    ...
    case SYSCALL_DRAWWINDOWBACKGROND:
    ...
    case SYSCALL_DRAWTEXT:
        return kDrawText(PARAM(0), PARAM(1), PARAM(2), PARAM(3), PARAM(4),
            (char*)PARAM(5), PARAM(6));
    ...
    case SYSCALL_TEST:
        ...
        // 정의되지 않은 시스템 콜 처리
    default:
        kPrintf("Undefined System Call, Service Number : %d", qwServiceNumber);
        return FALSE;
    }
}