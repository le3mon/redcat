...
// MINT 파일 시스템 할수를 표준 입출력 함수 이름으로 재정의
...
typedef struct kPartitionStruct {
    // 부팅 가능 플래그. 0x80 = 부팅 가능, 0x00 = 부팅 불가능
    BYTE bBootableFlag;

    // 파티션의 시작 어드레스. 현재는 사용 X 아래의 LBA 어드레스 대신 사용
    BYTE vbStartingCHSAddress[3];

    // 파티션 타입
    BYTE bPartitionType;

    // 파티션의 마지막 어드레스. 현재는 사용 X
    BYTE vbEndingCHSAddress[3];

    // 파티션의 시작 어드레스. LBA 어드레스로 나타난 값
