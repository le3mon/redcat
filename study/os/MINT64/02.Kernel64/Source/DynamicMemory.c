...
void kInitializeDynamicMemory(void) {
    ...
    for(j = 0; j < gs_stDynamicMemory.iMaxLevelCount; j++) {
        ...
        // 8개 이상 남아있으면 상위 블록으로 모두 결합할 수 있으므로, 모두 비어 있는 것으로 설정
        for(i = 0; i < iBlockCountOfLevel / 8; i++) {
            *pbCurrentBitmapPosition = 0x00;
            pbCurrentBitmapPosition++;
        }

        // 8로 나누어 떨어지지 않은 나머지 블록 처리
        if((iBlockCountOfLevel % 8) != 0) {
            *pbCurrentBitmapPosition = 0x00;

            // 남은 블럭이 홀수라면 마지막 한 블록은 결합되어 상위 블록으로 이동 못함
...
static QWORD kCalculateDynamicMemorySize(void) {
    ...
    // 3GB 이상의 메모리에는 비디오 메모리와 프로세서가 사용하는 레지스터가 존재하므로 최대 3GB 사용
...
void *kAllocateMemory(QWORD qwSize) {
    ...
    qwAligendSize = kGetBuddyBlockSize(qwSize);
    if(qwAligendSize == 0) {
        return NULL;
    }

    // 만약 여유 공간이 충분하지 않으면 실패
    if(gs_stDynamicMemory.qwStartAddress + gs_stDynamicMemory.qwUsedSize + qwAligendSize > gs_stDynamicMemory.qwEndAddress) {
        return NULL;
    }

    // 버디 블록 할당하고 할당된 블록이 속한 블록 리스트의 인덱스 반환
    lOffset = kAllocationBuddyBlock(qwAligendSize);
    if(lOffset == -1) {
        return NULL;
    }

    iIndexOfBlockList = kGetBlockListIndexOfMatchSize(qwAligendSize);

    // 블록 크기를 저장하는 영역에 실제로 할당된 버디 블록이 속한 블록 리스트의 인덱스를 저장
    // 메모리를 해제할 때 블록 리스트의 인덱스를 사용
    qwRelativeAddress = qwAligendSize * lOffset;
    ...
    gs_stDynamicMemory.qwUsedSize += qwAligendSize;
...
static int kGetBlockListIndexOfMatchSize(QWORD qwAligendSize) {
    ...
    for(i = 0; i < gs_stDynamicMemory.iMaxLevelCount; i++) {
        if(qwAligendSize <= (DYNAMICMEMORY_MIN_SIZE << i)) {
            return i;
        }
    }
...
static BOOL kFreeBuddyBlock(int iBlockListIndex, int iBlockOffset) {
    ...
    for(i = iBlockListIndex; i < gs_stDynamicMemory.iMaxLevelCount; i++) {
        ...
        // 블록의 오프셋이 짝수면 홀수를 검사하고,
        // 홀수면 짝수의 비트맵을 검사하여 인접한 블록이 존재한다면 결합
        if((iBlockOffset % 2) == 0) {
