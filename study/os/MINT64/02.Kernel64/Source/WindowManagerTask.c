...
void kStartWindowManager(void) {
    ...
    while(1) {
        ...
        // 이벤트 큐에 이벤트 처리 시 윈도우 크기 변경 표식이 지워질 수 있으므로 다시 출력
...
BOOL kProcessMouseData(void) {
    ...
    if(bChangeButton & MOUSE_LBUTTONDOWN) {
        ...
        // 왼쪽 버튼이 떨어진 경우 처리
            ...
            // 윈도우가 크기 변경 중이면 결정된 크기로 윈도우 크기 변경
                ...
                // 윈도우 매니저 자료구조에 저장된 영역을 이용하여 윈도우의 크기 변경
    ...
    // 윈도우가 크기 변경 중이면 크기 변경 처리
    else if(pstWindowManager->bWindowResizeMode == TRUE) {
        // 이전 위치 윈도우 크기 변경 표식 삭제
        kDrawResizeMarker(&(pstWindowManager->stResizingWindowArea), FALSE);

        // 마우스 이동 거리 이용하여 새로운 윈도우 크기 결정
...
// 비디오 메모리에 윈도우 크기 변경 표식을 출력하거나 출력된 표식 삭제
void kDrawResizeMarker(const RECT *pstArea, BOOL bShowMarker) {
    ...
    if(bShowMarker == TRUE) {
        ...
        kInternalDrawRect(&(pstWindowManager->stScreenArea),
            ...
            stMarkerArea.iX2, stMarkerArea.iY1 + WINDOWMANAGER_RESIZEMARKERSIZE,
            WINDOWMANAGER_COLOR_RESIZEMARKER, TRUE);
        kInternalDrawRect(&(pstWindowManager->stScreenArea),
            pstWindowManager->pstVideoMemory, stMarkerArea.iX1, stMarkerArea.iY1,
            stMarkerArea.iX1 + WINDOWMANAGER_RESIZEMARKERSIZE, stMarkerArea.iY2,
            WINDOWMANAGER_COLOR_RESIZEMARKER, TRUE);
        ...
        kInternalDrawRect(&(pstWindowManager->stScreenArea),
            ...
            stMarkerArea.iX2, stMarkerArea.iY2, WINDOWMANAGER_COLOR_RESIZEMARKER, TRUE);
        ...
        // 오른쪽 아래 표식
        kSetRectangleData(pstArea->iX2 - WINDOWMANAGER_RESIZEMARKERSIZE,
            pstArea->iY2 - WINDOWMANAGER_RESIZEMARKERSIZE, pstArea->iX2, pstArea->iY2, &stMarkerArea);
        kInternalDrawRect(&(pstWindowManager->stScreenArea),
            pstWindowManager->pstVideoMemory, stMarkerArea.iX1, stMarkerArea.iY2 -
            WINDOWMANAGER_THICK_RESIZEMARKER, stMarkerArea.iX2, stMarkerArea.iY2,
            WINDOWMANAGER_COLOR_RESIZEMARKER, TRUE);
    ...
    // 윈도우 크기 변경 표식을 삭제하는 경우
        ...
        // 오른쪽 아래 표식
        kSetRectangleData(pstArea->iX2 - WINDOWMANAGER_RESIZEMARKERSIZE,
            pstArea->iY2 - WINDOWMANAGER_RESIZEMARKERSIZE, pstArea->iX2,
            pstArea->iY2, &stMarkerArea);
        kRedrawWindowByArea(&stMarkerArea, WINDOW_INVALIDID);
    }
}