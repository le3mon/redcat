...
typedef struct kVBEInfoBlockStruct {
    // 모든 VBE 버전에 공통인 부분
    WORD wModeAttribute;    // 모드 속성
    BYTE bWinAAttribute;    // 윈도우 A 속성
    BYTE bWinBAttribute;    // 윈도우 B 속성
    WORD wWinGranulity;     // 윈도우의 가중치
    WORD wWinSize;          // 윈도우 크기
    ...
    // VBE 3.0 이상 공통 부분
    WORD wLinearBytesPerScanLine; // 선형 프레임 버퍼 모드의 화면 스캔 라인당 바이트 수
