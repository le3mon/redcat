...
static void kInstallPackage(const char *pcParameterBuffer) {
    ...
    // 부트 로더가 로딩된 0x7c05 어드레스에서 보호 모드 커널과 ida-32e 모드 커널을 합한 섹터 수 읽음
    ...
    // 시그니처 확인
    if(kMemCmp(pstHeader->vcSignature, PACKAGESINGNATURE, sizeof(pstHeader->vcSignature)) != 0) {
        kPrintf("Package Signature Fail\n");
        return ;
    }

    // 패키지 내의 모든 파일을 디스크에 복사
    // 패키지 데이턱 ㅏ시작하는 어드레스
