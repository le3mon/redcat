...
BOOL kFindMPFloatingPointerAddress(QWORD *pstAddress) {
    ...
    // 세그먼트 시작 어드레스 이므로 16을 곱하여 실제 물리 어드레스로 변환
    qwEBDAAddress *= 16;
...
BOOL kAnalysisMPConfigurationTable(void) {
    QWORD qwMPFlotingPointerAddress;
    ...
    // MP 플로팅 포인터 어드레스 구함
    if(kFindMPFloatingPointerAddress(&qwMPFlotingPointerAddress) == FALSE) {
        return FALSE;
    }

    // MP 플로팅 테이블 설정
    pstMPFloatingPointer = (MPFLOATINGPOINTER*)qwMPFlotingPointerAddress;
...
void kPrintMPConfigurationTable(void) {
    ...
    // MP 설정 테이블 처리 함수를 먼저 호출하여 시스템 처리에 필요한 정보 저장
    ...
    for(i = 0; i < pstMPTableHeader->wEntryCount; i++) {
        ...
        case MP_ENTRYTYPE_PROCESSOR:
            ...
            // 프로세스 엔트리의 크기만큼 증가시켜 다음 엔트리로 이동
        ...
        case MP_ENTRYTYPE_BUS:
            ...
            // 버스 엔트리으 ㅣ크기만큼 어드레스를 증가시켜 다음 엔트리로 이동
...
int kGetProcessorCount(void) {
    // MP 설정 테이블이 없을 수도 있으므로 0으로 설정되면 1 반환
