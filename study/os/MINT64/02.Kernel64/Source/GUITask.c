...
void kBaseGUITask(void) {
    ...
    // 그래픽 모드 판단
    // MINT 64 OS가 그래픽 모드로 시작한지 확인
    if(kIsGraphicMode() == FALSE) {
        // MINT64 OS가 그래픽 모드로 시작하지 않으면 실패
    ...
    // 윈도우 생성 함수 호출, 마우스가 있던 위치 기준으로 생성
    qwWindowID = kCreateWindow(iMouseX - 10, iMouseY - WINDOW_TITLEBAR_HEIGHT / 2,
        iWindowWidth, iWindowHeight, WINDOW_FLAGS_DEFAULT | WINDOW_FLAGS_RESIZEABLE,
        "Hello World Window");
    ...
    while(1) {
        ...
        case EVENT_WINDOW_CLOSE:
            ...
            // 윈도우 닫기 이벤트일 경우 윈도우 삭제 후 루프 빠져나가 태스크 종료
...
void kHelloWorldGUITask(void) {
    ...
    // 윈도우 생성 함수 호출, 마우스 위치 기준으로 생성
    // 번호를 추가하여 윈도우마다 개별적인 이름 할당
    ...
    // 이벤트 정보 표시하는 영역의 테두리와 윈도우 id 표시
        ...
        case EVENT_MOUSE_MBUTTONUP:
            ...
            if(stReceivedEvent.qwType == EVENT_MOUSE_LBUTTONDOWN) {
                ...
                if(kIsInRectangle(&stButtonArea, pstMouseEvent->stPoint.iX,
                    ...
                    // 생성된 윈도우의 수만큼 루프를 수행하면서 이벤트를 전송
...
void kSystemMonitorTask(void) {
    ...
    // 그래픽 모드 판단
    // MINT64 OS가 그래픽 모드로 시작한지 확인
    if(kIsGraphicMode() == FALSE) {
        // 그래픽 모드로 시작하지 않을 시 실패
        kPrintf("This task can run only GUI Mode~!!!\n");
        return;
    }
    ...
    // 윈도우를 화면 가운데 생성한 뒤 화면에 표시하지 않음
    // 프로세서 정보와 메모리 정보를 표시하는 영역을 그린 뒤 화면에 표시
    ...
    // 프로세서 정보를 표시하는 영역을 3픽셀 두께로 표시하고 문자열 출력
    ...
    // 메모리 정보를 표시하는 영역을 3픽셀 두께로 표시하고 문자열 출력
    ...
    // 마지막으로 측정한 ㅍ로세서의 부하와 태스크 수, 그리고 메모리 사용량은 모두 0으로 설정
    ...
    while(1) {
        ...
        if(kReceiveEventFromWindowQueue(qwWindowID, &stReceivedEvent) == TRUE) {
            ...
            switch(stReceivedEvent.qwType) {
                // 윈도우 이벤트 처리
            case EVENT_WINDOW_CLOSE:
                // 윈도우 닫기 이벤트이면 윈도우 삭제 후 루프를 빠져나가 태스크 종료
                kDeleteWindow(qwWindowID);
                return;
                break;
        ...
        // 0.5초마다 한번씩 시스템 상태 확인
        if((kGetTickCount() - qwLastTickCount) < 500) {
            kSleep(1);
            continue;
        }

        // 마지막으로 측정한 시간 최신으로 업데이트
        qwLastTickCount = kGetTickCount();

        // 프로세서 정보 출력
        // 프로세서 수만큼 부하와 태스크 수 확인하여 달라진 부분 화면에 업데이트
        for(i = 0; i < iProcessorCount; i++) {
            ...
            // 이전과 비교하여 변경 사항이 있을 시 화면에 업데이트
            if(bChanged == TRUE) {
                // 화면에 현재 프로세서 부하 표시
                kDrawProcessorInformation(qwWindowID, i * SYSTEMMONITOR_PROCESSOR_WIDTH +
                    (i + 1) * SYSTEMMONITOR_PROCESSOR_MARGIN, WINDOW_TITLEBAR_HEIGHT + 28, i);
            }
        }
        ...
        // 현재 동적 할당 메모리 사용량이 이전과 다르다면 메모리 정보 출력
...
static void kDrawProcessorInformation(QWORD qwWindowID, int iX, int iY, BYTE bAPICID) {
    ...
    // 프로세서 부하를 표시, 막대의 가운데에 부하가 표시되도록 함
...
static void kDrawMemoryInformation(QWORD qwWindowID, int iY, int iWindowWidth) {
    ...
    // 메모리 사용량을 나타내는 막대의 길이, (막대 전체 길이 * 메모리 사용량 / 100)
    iUsageBarWidth = (iWindowWidth - 2 * SYSTEMMONITOR_PROCESSOR_MARGIN) *
        qwUsedPercent / 100;
    
    // 메모리 사용량을 표시하는 영역의 막대 내부 표시
    // 색칠된 막대 표시, 테두리와 2픽셀 정도 여유 공간 둠
    ...
    // 사용량을 나타내는 텍스트 표시, 막대의 가운데 사용량 표시
...
void kGUIConsoleShellTask(void) {
    ...
    // 윈도우 크기 설정, 화면 버퍼에 들어가는 문자의 최대 너비와 높이 고려해서 계산
    ...
    while(1) {
        ...
        case EVENT_KEY_UP:
            ...
            // 키를 그래픽 모드용 키 큐로 삽입하여 셸 태스크 입력으로 전달
            kPutKeyToGUIKeyQueue(&stKeyData);
            break;
        
            // 윈도우 이벤트 처리
        case EVENT_WINDOW_CLOSE:
            // 생성할 셸 태스크 종료되도록 종료 플래그 설정 후 종료까지 대기
...
static void kProcessConsoleBuffer(QWORD qwWindowID) {
    ...
    // 화면 전체를 업데이트한 지 5초가 지났으면 무조건 화면 전체 다시 그림
    ...
    for(j = 0; j < CONSOLE_HEIGHT; j++) {
        ...
        // 현재 라인에서 변경된 데이터가 있다면 라인만 화면에 업데이트
        if(bChanged == TRUE) {
            // 현재 라인 영역 계산
...
void kImageViewerTask(void) {
    ...
    // 윈도우 크기 설정, 화면 버퍼에 들어가는 문자 최대 너비와 높이 고려해서 계산
    ...
    // 파일 이름 버퍼 비우고 에디트 박스에 빈 파일 이름 표시
    iFileNameLength = 0;
    ...
    while(1) {
        ...
        case EVENT_MOUSE_LBUTTONDOWN:
            ...
            // 마우스 왼쪽 버튼이 이미지 출력 버튼 위에 눌러졌으면 저장된 파일 이름을
            // 이용하여 이미지 화면에 표시
            if(kIsInRectangle(&stButtonArea, pstMouseEvent->stPoint.iX,
                pstMouseEvent->stPoint.iY) == TRUE) {
                // 버튼을 눌린 것으로 표시
                ...
                if(kCreateImageViewerWindowAndExecute(qwWindowID, vcFileName) == FALSE) {
                    // 윈도우 생성에 실패하면 버튼이 눌러졌다 떨어지는 효과를 주기 위해
                    // 200ms 대기
                    kSleep(200);
                }
        ...
        case EVENT_KEY_DOWN:
            ...
            // 엔터 키는 이미지 출력 버튼이 눌린 것으로 처리
                ...
                (iFileNameLength > 0)) {
                // 버튼의 XY 좌표를 화면 좌표로 변환하여 마우스 이벤트의 좌표로 이동
                stClientXY.iX = stButtonArea.iX1 + 1;
                stClientXY.iY = stButtonArea.iY1 + 1;
                kConvertPointClientToScreen(qwWindowID, &stClientXY, &stScreenArea);
...
static void kDrawFileName(QWORD qwWindowID, RECT *pstArea, char *pcFileName, int iNameLength) {
    ...
    // 파일 이름의 길이가 파일 시스템이 정의한 최대 길이가 아니면 커서 출력
...
// JPEG 파일을 읽어서 새로 생성한 윈도우에 표시하고 이벤트 처리
static BOOL kCreateImageViewerWindowAndExecute(QWORD qwMainWindowID, const char *pcFileName) {
    ...
    if((pbFileBuffer == NULL) || (pstJpeg == NULL)) {
        kPrintf("[ImageViewer] Buffer allocaton fail\n");
    ...
    if((pstOutputBuffer != NULL) &&
        ...
        qwWindowID = kCreateWindow((stScreenArea.iX2 - pstJpeg->width) / 2,
            ...
            WINDOW_FLAGS_DEFAULT & ~WINDOW_FLAGS_SHOW | WINDOW_FLAGS_RESIZEABLE,
            pcFileName);
    }

    // 윈도우 생성에 실패하거나 출력 버퍼 할당 또는 디코딩 실패 시 종료
    ...
    // 윈도우 너비를 구하여 제목 표시줄 영역을 제외한 나머지 화면 버퍼 영역에 디코딩된 이미지 복사
    ...
    while(bExit == FALSE) {
        ...
        case EVENT_KEY_DOWN:
            ...
            // ESC가 눌리면 그림을 표시하는 윈도우를 삭제 후 파일 이름 입력 윈도우 표시 후 종료
